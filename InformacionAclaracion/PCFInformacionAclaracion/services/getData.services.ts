import { DataIncident } from './../interfaces/general';
...
export class DataService {
  ...
  async getDataAclaracion(): Promise<DataIncident> {
    let reqData = await this.requestGet("/api/data/v9.2/incidents(" + this.getIncidentId()
      + ")?$select=_createdby_value,_cxm_categoriaid_value,_cxm_subcategoriaid_value,casetypecode,_cxm_motornormativoid_value,cxm_cadenanormativa,cxm_montototal,cxm_fechaestimadacierre,numberofchildincidents,cxm_sys_jsonmovimiento,cxm_sys_jsonproducto,cxm_tipocasoalta,cxm_tipoestatustransaccion,cxm_motornormativodescripcion,cxm_estatusplastico,cxm_fechaestatusplastico,cxm_tipoabono,cxm_tipocargo,_cxm_usuarioaltaid_value,_cxm_unidadnegocioid_value,cxm_vozcliente,_customerid_value")
    if (reqData.ok)
      return reqData.data
      ...
      return {
        ...
        _customerid_value: "8350ade1-813a-ed11-9db0-000d3a36b2ec"
      }
    return {} as any;
  }
  ...
  async getDataIndicadoresCliente(clientId: string, fecha: string): Promise<any> {
    let reqData = await this.requestGet("/api/data/v9.2/cxm_mx_det_acl_indicadorclientes"
      + "?$select=_cxm_contactid_value,createdon,_cxm_indicadorid_value"
      + "&$filter=(_cxm_contactid_value eq " + clientId + " and createdon ge " + fecha + "T00:00:00.000Z)")
    if (reqData.ok)
      return reqData.data
    else if (typeof Xrm === "undefined")
      return {
